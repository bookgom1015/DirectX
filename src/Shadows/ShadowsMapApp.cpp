#include "Shadows/ShadowsMapApp.h"
...
#if defined(Ex2)
	mSceneBounds.Radius = sqrtf(5.0f * 5.0f + 7.6f * 7.5f);
#else
...
#elif defined(Ex5_1024)
		mShadowMap = std::make_unique<ShadowMap>(md3dDevice.Get(), 1024, 1024);
#elif defined(Ex5_512)
		mShadowMap = std::make_unique<ShadowMap>(md3dDevice.Get(), 512, 512);
#elif defined(Ex5_256)
		mShadowMap = std::make_unique<ShadowMap>(md3dDevice.Get(), 256, 256);
#else
...
void ShadowMapApp::Draw(const GameTimer& gt) {
	...
	CD3DX12_GPU_DESCRIPTOR_HANDLE projectMapDescriptor(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
	projectMapDescriptor.Offset(mProjectMapHeapIndex, mCbvSrvUavDescriptorSize);
	mCommandList->SetGraphicsRootDescriptorTable(5, projectMapDescriptor);
#endif
...
#if defined(Ex1_) || defined(Ex4)
	XMMATRIX lightProj = XMMatrixPerspectiveFovLH(0.25f*MathHelper::Pi, 1.0f, 5.0f, 1000.0f);
#else
...
void ShadowMapApp::UpdateMainPassCB(const GameTimer& gt) {
	...
	mMainPassCB.Lights[0].FalloffEnd = 100.0f;
	mMainPassCB.Lights[0].SpotPower = 128.0f;
#else
	mMainPassCB.Lights[0].Direction = mRotatedLightDirections[0];
...
void ShadowMapApp::BuildRootSignature() {
	...
	texTable2.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0, 2);
...
#if defined(Ex1) || defined(Ex2) || defined(Ex3)
	const UINT numParameters = 6;
#else
	const UINT numParameters = 5;
#endif
...
#if defined(Ex1) || defined(Ex2) || defined(Ex3)
		mTextures["projectDiffuseMap"]->Resource
#endif
	};
...
#if defined(Ex1) || defined(Ex2) || defined(Ex3)
	mProjectMapHeapIndex = mSkyTexHeapIndex - 1;
#endif
...
#elif defined(Ex10)
	smapPsoDesc.RasterizerState.DepthBias = 1000000;
	smapPsoDesc.RasterizerState.SlopeScaledDepthBias = 1.0f;
	smapPsoDesc.RasterizerState.DepthBiasClamp = 1.0f;
#else
