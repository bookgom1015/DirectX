#include "TreeBillboards/TreeBillboards.h"
...
#
	...
	DrawRenderItems(mCommandList.Get(), mRitemLayer[static_cast<int>(RenderLayer::Line)]);
#elif defined(Ex2)
...
void TreeBillboardsApp::BuildDescriptorHeaps() {
	...
	_ASSERT(mTextures["grassTex"]);
	auto grassTex = mTextures["grassTex"]->Resource;
	_ASSERT(mTextures["waterTex"]);
	auto waterTex = mTextures["waterTex"]->Resource;
#if defined(Ex1) || defined(Ex2) || defined(Ex3) || defined(Ex4) || defined(Ex5)
	_ASSERT(mTextures["whiteTex"]);
	auto whiteTex = mTextures["whiteTex"]->Resource;
#else
	_ASSERT(mTextures["fenceTex"]);
	auto fenceTex = mTextures["fenceTex"]->Resource;
#endif
	_ASSERT(mTextures["treeArrayTex"]);
...
#if defined(Ex6_a) || defined(Ex6_b)
	mShaders["treeSpriteVS"] = d3dUtil::CompileShader(L"Shaders\\TreeSpriteID.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["treeSpriteGS"] = d3dUtil::CompileShader(L"Shaders\\TreeSpriteID.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["treeSpritePS"] = d3dUtil::CompileShader(L"Shaders\\TreeSpriteID.hlsl", alphaTestDefines, "PS", "ps_5_0");
#elif defined(Ex7_a) || defined(Ex7_b)
	mShaders["treeSpriteVS"] = d3dUtil::CompileShader(L"Shaders\\TreeSpriteIdx.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["treeSpriteGS"] = d3dUtil::CompileShader(L"Shaders\\TreeSpriteIdx.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["treeSpritePS"] = d3dUtil::CompileShader(L"Shaders\\TreeSpriteIdx.hlsl", alphaTestDefines, "PS", "ps_5_0");
#else
...
#if defined(Ex1)
	mShaders["circleVS"] = d3dUtil::CompileShader(L"Shaders\\CirtoCyl.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["circleGS"] = d3dUtil::CompileShader(L"Shaders\\CirtoCyl.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["circlePS"] = d3dUtil::CompileShader(L"Shaders\\CirtoCyl.hlsl", nullptr, "PS", "ps_5_0");
#elif defined(Ex2)
	mShaders["lineVS"] = d3dUtil::CompileShader(L"Shaders\\LOD.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["lineGS"] = d3dUtil::CompileShader(L"Shaders\\LOD.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["linePS"] = d3dUtil::CompileShader(L"Shaders\\LOD.hlsl", nullptr, "PS", "ps_5_0");
#elif defined(Ex3)
	mShaders["explosionVS"] = d3dUtil::CompileShader(L"Shaders\\Explosion.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["explosionGS"] = d3dUtil::CompileShader(L"Shaders\\Explosion.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["explosionPS"] = d3dUtil::CompileShader(L"Shaders\\Explosion.hlsl", nullptr, "PS", "ps_5_0");
#elif defined(Ex4)
...
#elif defined(Ex5)
	mShaders["normalVS"] = d3dUtil::CompileShader(L"Shaders\\TriNormal.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["normalGS"] = d3dUtil::CompileShader(L"Shaders\\TriNormal.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["normalPS"] = d3dUtil::CompileShader(L"Shaders\\TriNormal.hlsl", nullptr, "PS", "ps_5_0");
#endif
...
void TreeBillboardsApp::BuildBoxGeometry() {
#if defined(Ex1)
	static const int sliceCount = 16;
	static const float radius = 4.0f;

	std::vector<Vertex> vertices(sliceCount);
	...
	std::vector<std::uint16_t> indices(sliceCount + 1);
	for (size_t i = 0; i < sliceCount; i++)
	...
	geo->IndexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(), mCommandList.Get(),
		indices.data(), ibByteSize, geo->VertexBufferUploader);
...
#elif defined(Ex2)
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData geoSphere = geoGen.CreateGeosphere(1.0f, 0);
	...
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData sphere = geoGen.CreateSphere(1.0f, 8, 8);
...
#if defined(Ex3)
	opaquePsoDesc.VS = {
		reinterpret_cast<BYTE*>(mShaders["explosionVS"]->GetBufferPointer()),
		mShaders["explosionVS"]->GetBufferSize()
	};
	opaquePsoDesc.GS = {
		reinterpret_cast<BYTE*>(mShaders["explosionGS"]->GetBufferPointer()),
		mShaders["explosionGS"]->GetBufferSize()
	};
	opaquePsoDesc.PS = {
		reinterpret_cast<BYTE*>(mShaders["explosionPS"]->GetBufferPointer()),
		mShaders["explosionPS"]->GetBufferSize()
	};
...
#elif defined(Ex2)
	D3D12_GRAPHICS_PIPELINE_STATE_DESC linePsoDesc = opaquePsoDesc;
	linePsoDesc.VS = {
		reinterpret_cast<BYTE*>(mShaders["lineVS"]->GetBufferPointer()),
		mShaders["lineVS"]->GetBufferSize()
	};
	linePsoDesc.GS = {
		reinterpret_cast<BYTE*>(mShaders["lineGS"]->GetBufferPointer()),
		mShaders["lineGS"]->GetBufferSize()
	};
	linePsoDesc.PS = {
		reinterpret_cast<BYTE*>(mShaders["linePS"]->GetBufferPointer()),
		mShaders["linePS"]->GetBufferSize()
	};
	//linePsoDesc.RasterizerState.FillMode = D3D12_FILL_MODE_WIREFRAME;
	linePsoDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
...
void TreeBillboardsApp::BuildRenderItems() {
	...
	XMStoreFloat4x4(&circleRitem->World, XMMatrixTranslation(0.0f, 8.0f, 0.0f));
	circleRitem->ObjCBIndex = 2;
	_ASSERT(mMaterials["white"]);
	circleRitem->Mat = mMaterials["white"].get();
	_ASSERT(mGeometries["circleGeo"]);
	circleRitem->Geo = mGeometries["circleGeo"].get();
	circleRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP;
	auto iter = circleRitem->Geo->DrawArgs.find("circle");
	_ASSERT(iter != circleRitem->Geo->DrawArgs.end());
	...
	mRitemLayer[static_cast<UINT>(RenderLayer::Line)].push_back(circleRitem.get());
#elif defined(Ex2)
	auto sphereRitem = std::make_unique<RenderItem>();
	sphereRitem->ObjCBIndex = 2;
	_ASSERT(mMaterials["white"]);
	sphereRitem->Mat = mMaterials["white"].get();
	_ASSERT(mGeometries["geoSphereGeo"]);
	sphereRitem->Geo = mGeometries["geoSphereGeo"].get();
	sphereRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	auto iter = sphereRitem->Geo->DrawArgs.find("geoSphere");
	_ASSERT(iter != sphereRitem->Geo->DrawArgs.end());
	sphereRitem->IndexCount = sphereRitem->Geo->DrawArgs["geoSphere"].IndexCount;
	sphereRitem->StartIndexLocation = sphereRitem->Geo->DrawArgs["geoSphere"].StartIndexLocation;
	sphereRitem->BaseVertexLocation = sphereRitem->Geo->DrawArgs["geoSphere"].BaseVertexLocation;

	mRitemLayer[static_cast<UINT>(RenderLayer::Line)].push_back(sphereRitem.get());
#elif defined(Ex3) || defined(Ex4) || defined(Ex5)
	auto sphereRitem = std::make_unique<RenderItem>();
	sphereRitem->ObjCBIndex = 2;
	_ASSERT(mMaterials["white"]);
	sphereRitem->Mat = mMaterials["white"].get();
	_ASSERT(mGeometries["sphereGeo"]);
	sphereRitem->Geo = mGeometries["sphereGeo"].get();
	sphereRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	auto iter = sphereRitem->Geo->DrawArgs.find("sphere");
	_ASSERT(iter != sphereRitem->Geo->DrawArgs.end());
	sphereRitem->IndexCount = sphereRitem->Geo->DrawArgs["sphere"].IndexCount;
	sphereRitem->StartIndexLocation = sphereRitem->Geo->DrawArgs["sphere"].StartIndexLocation;
	sphereRitem->BaseVertexLocation = sphereRitem->Geo->DrawArgs["sphere"].BaseVertexLocation;

	mRitemLayer[static_cast<UINT>(RenderLayer::Opaque)].push_back(sphereRitem.get());
#else
