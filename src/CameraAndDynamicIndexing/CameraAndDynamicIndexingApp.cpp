#include "CameraAndDynamicIndexing/CameraAndDynamicIndexingApp.h"
...
void CameraAndDynamicIndexingApp::OnKeyboardInput(const GameTimer& gt) {
	...
	if (GetAsyncKeyState('Q') & 0x8000)
		mCamera.Roll(XMConvertToRadians(60.0f * gt.DeltaTime()));
...
void CameraAndDynamicIndexingApp::BuildMaterials() {
	...
	auto check = std::make_unique<Material>();
	check->Name = "check";
	check->MatCBIndex = 1;
	check->DiffuseSrvHeapIndex = 1;
	check->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	check->FresnelR0 = XMFLOAT3(0.02f, 0.02f, 0.02f);
	check->Roughness = 0.3f;
	...
	mMaterials[check->Name] = std::move(check);
...
#if defined(Ex3)
		cmdList->DrawIndexedInstanced(ri->IndexCount, 5, ri->StartIndexLocation, ri->BaseVertexLocation, 0);
#else
