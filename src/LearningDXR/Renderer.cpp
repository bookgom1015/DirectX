...
Renderer::Renderer() {
	...
	mDXRObjectCB = std::make_unique<DXRObjectCB>();
	mBLAS = std::make_unique<AccelerationStructureBuffer>();
...
bool Renderer::Initialize(HWND hMainWnd, UINT width, UINT height) {
	...
	// Ray-tracing
	CheckIsValid(CompileDXRShaders());
	CheckIsValid(BuildDXRRootSignatures());
	CheckIsValid(BuildDXRResources());
	CheckIsValid(BuildDXRDescriptorHeaps());
	CheckIsValid(BuildDXRDescriptors());
	CheckIsValid(BuildBLAS());
	CheckIsValid(BuildTLAS());
	CheckIsValid(BuildDXRPSOs());
	CheckIsValid(BuildDXRObjectCB());
...
bool Renderer::CompileShaders() {
	...
	{
		const std::wstring filePath = ShaderFilePathW + L"LearningDXR\\Gizmo.hlsl";
...
bool Renderer::CompileDXRShaders() {
	{
...
bool Renderer::BuildDXRRootSignatures() {
	...
	//
	{
		...
		params[static_cast<int>(ELocalRootSignatureParams::EObjectCB)].InitAsConstants(D3D12Util::CalcNumUintValues<DXRObjectCB>(), 1);
		//params[static_cast<int>(ELocalRootSignatureParams::EMatCB)].InitAsConstantBufferView(2);
...
bool Renderer::BuildDXRResources() {
...
bool Renderer::BuildDXRDescriptorHeaps() {
	D3D12_DESCRIPTOR_HEAP_DESC desc = {};
	// 3 UAV for the RT output
	// 2 SRV for vertex and index buffers
	desc.NumDescriptors = gNumFrameResources + 2;
	desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	desc.NodeMask = 0;
	
	CheckHResult(md3dDevice->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&mDXRDescriptorHeap)));
	
	return true;
}

bool Renderer::BuildDXRDescriptors() {
	...
	for (size_t i = 0; i < gNumFrameResources; ++i) {
		md3dDevice->CreateUnorderedAccessView(mDXROutputs[i].Get(), nullptr, &uavDesc, handle);
		handle.ptr += cbvSrvUavDescriptorSize;
	}
...
bool Renderer::BuildBLAS() {
	...
	CheckIsValid(D3D12Util::CreateBuffer(md3dDevice.Get(), bufferInfo, mBLAS->Scratch.GetAddressOf(), mInfoQueue.Get()));
	...
	CheckIsValid(D3D12Util::CreateBuffer(md3dDevice.Get(), bufferInfo, mBLAS->Result.GetAddressOf(), mInfoQueue.Get()));
...
bool Renderer::BuildDXRPSOs() {
	...
	//ReturnIfFailedDxDebug(mInfoQueue.Get(), md3dDevice->CreateStateObject(dxrPso, IID_PPV_ARGS(&mDXRPSOs["default"])));
	//ReturnIfFailedDxDebug(mInfoQueue.Get(), mDXRPSOs["default"]->QueryInterface(IID_PPV_ARGS(&mDXRPSOProps["defaultProps"])));
...
bool Renderer::BuildDXRObjectCB() {
...
bool Renderer::BuildShaderTables() {
	...
	CheckIsValid(hitGroupTable.Initialze());
	{
		...
		struct RootArguments {
			DXRObjectCB ObjectCB;
		} rootArguments;

		rootArguments.ObjectCB = *mDXRObjectCB;
...
bool Renderer::Raytrace() {
	...
	mCommandList->SetComputeRootDescriptorTable(
		...
		D3D12Util::GetGpuHandle(mDXRDescriptorHeap.Get(), static_cast<INT>(EDXRDescriptors::EOutputUAV) + mCurrFrameResourceIndex, cbvSrvUavDescriptorSize)
	);	
	mCommandList->SetComputeRootDescriptorTable(
		static_cast<UINT>(EGlobalRootSignatureParams::EGeometryBuffers),
		D3D12Util::GetGpuHandle(mDXRDescriptorHeap.Get(), static_cast<INT>(EDXRDescriptors::EGeomtrySRVs), cbvSrvUavDescriptorSize)
	);
	...
	const auto output = mDXROutputs[mCurrFrameResourceIndex].Get();
	...
	mCommandList->CopyResource(CurrentBackBuffer(), output);
