#include "InstancingAndCulling/InstancingAndCullingApp.h"
...
#if defined(Ex1)
			if ((localSpaceFrustum.Contains(e->Sphere) != DirectX::DISJOINT) || (mFrustumCullingEnabled == false)) 
#else
			if ((localSpaceFrustum.Contains(e->Bounds.AABB) != DirectX::DISJOINT) || (mFrustumCullingEnabled == false))
#endif
			{
				InstanceData data;
...
void InstancingAndCullingApp::BuildSkullGeometry() {
	...
	XMStoreFloat3(&bounds.Center, vCenter);

	XMVECTOR vRadius = XMVectorMax(vCenter - vMin, vCenter - vMax);
	vRadius = XMVector4Length(vRadius);

	bounds.Radius = vRadius.m128_f32[0];
#else
	BoundingBox bounds;
...
#if defined(Ex1)
	skullRitem->Sphere = skullRitem->Geo->DrawArgs["skull"].Sphere;
#else
	skullRitem->AABB = skullRitem->Geo->DrawArgs["skull"].AABB;
#endif
