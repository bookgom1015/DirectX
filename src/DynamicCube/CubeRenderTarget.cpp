...
CD3DX12_GPU_DESCRIPTOR_HANDLE CubeRenderTarget::Srv(int faceIndex) {
#if defined(Ex4)
	return mhTexArrayGpuSrv[faceIndex];
#else
	return mhGpuSrv;
#endif
}
...
void CubeRenderTarget::BuildDescriptors(CD3DX12_CPU_DESCRIPTOR_HANDLE hCpuSrv, 
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE hTexArrayCpuSrv[6],
	CD3DX12_GPU_DESCRIPTOR_HANDLE hTexArrayGpuSrv[6]) {
#if defined(Ex4)
	// Save references to the descriptors. 
	mhCpuSrv = hCpuSrv;
	mhGpuSrv = hGpuSrv;
	...
	for (int i = 0; i < 6; ++i) {
		mhCpuRtv[i] = hCpuRtv[i];
		mhTexArrayCpuSrv[i] = hTexArrayCpuSrv[i];
		mhTexArrayGpuSrv[i] = hTexArrayGpuSrv[i];
	}
...
void CubeRenderTarget::BuildDescriptors() {
	...
	for(int i = 0; i < 6; ++i) {
#if defined(Ex4)
		D3D12_SHADER_RESOURCE_VIEW_DESC texArrayDesc = {};
		texArrayDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
		texArrayDesc.Format = mFormat;
		texArrayDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;
		texArrayDesc.Texture2DArray.MipLevels = 1;
		texArrayDesc.Texture2DArray.MostDetailedMip = 0;
		texArrayDesc.Texture2DArray.PlaneSlice = 0;
		texArrayDesc.Texture2DArray.ResourceMinLODClamp = 0.0f;

		// Render target to ith element.
		texArrayDesc.Texture2DArray.FirstArraySlice = i;
		// Only view one element of the array.
		texArrayDesc.Texture2DArray.ArraySize = 1;

		// Create SRV to ith cubemap face.
		md3dDevice->CreateShaderResourceView(mCubeMap.Get(), &texArrayDesc, mhTexArrayCpuSrv[i]);
#endif
