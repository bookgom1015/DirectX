#include "DynamicCube/DynamicCubeApp.h"
...
#if defined(Ex3)
	mCommandList->SetPipelineState(mPSOs["Ex3"].Get());
...
void DynamicCubeMapApp::BuildPostProcessRootSignature() {
#if defined(Ex4)
...
void DynamicCubeMapApp::BuildDescriptorHeaps() {
...
#if defined(Ex4)
	CD3DX12_CPU_DESCRIPTOR_HANDLE texArraySrvCpuHandles[6];
	CD3DX12_GPU_DESCRIPTOR_HANDLE texArraySrvGpuHandles[6];

	for (int i = 0; i < 6; ++i) {
		UINT texArraySrvHeapIndex = mDynamicTexHeapIndex + 1;
		texArraySrvCpuHandles[i] = CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, texArraySrvHeapIndex + i, mCbvSrvUavDescriptorSize);
		texArraySrvGpuHandles[i] = CD3DX12_GPU_DESCRIPTOR_HANDLE(srvGpuStart, texArraySrvHeapIndex + i, mCbvSrvUavDescriptorSize);
	}
	...
	mDynamicCubeMap->BuildDescriptors(
		...
		cubeRtvHandles,
		texArraySrvCpuHandles,
		texArraySrvGpuHandles);
#else
	// Dynamic cubemap SRV is after the sky SRV.
	mDynamicCubeMap->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, mDynamicTexHeapIndex, mCbvSrvUavDescriptorSize),
...
#if defined(Ex3)
	mShaders["opaquePS_Ex3"] = d3dUtil::CompileShader(L"Shaders\\DefaultEx3.hlsl", nullptr, "PS", "ps_5_1");
#elif defined(Ex4)
...
#if defined(Ex3)
	D3D12_GRAPHICS_PIPELINE_STATE_DESC dynamicReflect = opaquePsoDesc;

	dynamicReflect.PS = {
		reinterpret_cast<BYTE*>(mShaders["opaquePS_Ex3"]->GetBufferPointer()),
		mShaders["opaquePS_Ex3"]->GetBufferSize()
	};
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&dynamicReflect, IID_PPV_ARGS(&mPSOs["Ex3"])));
#elif defined(Ex4)
...
void DynamicCubeMapApp::BuildMaterials() {
	...
	mirror0->FresnelR0 = XMFLOAT3(0.3f, 0.3f, 0.3f);
