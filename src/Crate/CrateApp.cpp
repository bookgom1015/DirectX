...
#if defined(Ex4_Trans)
	auto woodCrateMat = mMaterials["woodCrate"].get();

	woodCrateMat->MatTransform = MathHelper::Identity4x4();
	XMStoreFloat4x4(&woodCrateMat->MatTransform, XMMatrixTranslation(-0.5f, -0.5f, 0.0f) *
		XMMatrixRotationZ(gt.TotalTime()) * XMMatrixTranslation(0.5f, 0.5f, 0.0f));

	woodCrateMat->NumFramesDirty = gNumFrameResources;
#endif
}
...
void CrateApp::LoadTextures() {
	...
	mTextures[alphaTex->Name] = std::move(alphaTex);
#elif defined(Ex6)
	...
	mTextures[cylTex->Name] = std::move(cylTex);
#else
...
void CrateApp::BuildDescriptorHeaps() {
	...
	srvDesc1.Format = flareTex->GetDesc().Format;
	srvDesc1.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc1.Texture2D.MostDetailedMip = 0;
	srvDesc1.Texture2D.MipLevels = flareTex->GetDesc().MipLevels;
	srvDesc1.Texture2D.ResourceMinLODClamp = 0.0f;
	md3dDevice->CreateShaderResourceView(flareTex.Get(), &srvDesc1, hDescriptor);

	hDescriptor.Offset(1, mCbvSrvDescriptorSize);
	auto alphaTex = mTextures["alphaTex"]->Resource;
	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc2 = {};
	srvDesc2.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	srvDesc2.Format = alphaTex->GetDesc().Format;
	srvDesc2.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc2.Texture2D.MostDetailedMip = 0;
	srvDesc2.Texture2D.MipLevels = alphaTex->GetDesc().MipLevels;
	srvDesc2.Texture2D.ResourceMinLODClamp = 0.0f;
	md3dDevice->CreateShaderResourceView(alphaTex.Get(), &srvDesc2, hDescriptor);
#elif defined(Ex6)
	...
	srvDesc.Texture2D.MipLevels = boxTex->GetDesc().MipLevels;
	...
	auto cylTex = mTextures["cylTex"]->Resource;
	hDescriptor.Offset(1, mCbvSrvDescriptorSize);
	srvDesc.Format = cylTex->GetDesc().Format;
	srvDesc.Texture2D.MipLevels = cylTex->GetDesc().MipLevels;
	md3dDevice->CreateShaderResourceView(cylTex.Get(), &srvDesc, hDescriptor);
#else
...
void CrateApp::BuildMaterials() {
	...
	boxMat->FresnelR0 = XMFLOAT3(0.6f, 0.6f, 0.6f);
	...
	gridMat->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	gridMat->FresnelR0 = XMFLOAT3(0.6f, 0.6f, 0.6f);
	gridMat->Roughness = 0.2f;

	mMaterials[gridMat->Name] = std::move(gridMat);

	auto sphMat = std::make_unique<Material>();
	sphMat->Name = "sphMat";
	sphMat->MatCBIndex = 2;
	sphMat->DiffuseSrvHeapIndex = 0;
	sphMat->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	sphMat->FresnelR0 = XMFLOAT3(0.6f, 0.6f, 0.6f);
	sphMat->Roughness = 0.2f;

	mMaterials[sphMat->Name] = std::move(sphMat);

	auto cylMat = std::make_unique<Material>();
	cylMat->Name = "cylMat";
	cylMat->MatCBIndex = 3;
	cylMat->DiffuseSrvHeapIndex = 2;
	cylMat->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	cylMat->FresnelR0 = XMFLOAT3(0.6f, 0.6f, 0.6f);
	cylMat->Roughness = 0.2f;

	mMaterials[cylMat->Name] = std::move(cylMat);
#else
...
void CrateApp::BuildRenderItems() {
	...
	XMStoreFloat4x4(&gridRitem->Mat->MatTransform, XMMatrixScaling(8.0f, 8.0f, 8.0f));
	...
	for (int i = 0; i < 5; i++) {
		...
		leftCylRitem->Mat = mMaterials["cylMat"].get();
		...
		rightCylRitem->Mat = mMaterials["cylMat"].get();
		...
		leftSphereRitem->Mat = mMaterials["sphMat"].get();
		...
		rightSphereRitem->Mat = mMaterials["sphMat"].get();
...
#if defined(Ex1_Wrap) || defined(Ex1_Border) || defined(Ex1_Clamp) || defined(Ex1_Mirror)
	XMStoreFloat4x4(&boxRitem->TexTransform, XMMatrixScaling(3.0f, 3.0f, 1.0f) * XMMatrixTranslation(-0.5f, -0.5f, 0.0f));
#endif
	boxRitem->ObjCBIndex = 0;
	...
	// All the render tiems are opaque.
	for (auto& e : mAllRitems)
...
#if defined(Ex1_Linear)
		0, // shaderRegister
		D3D12_FILTER_MIN_MAG_MIP_LINEAR, // filter
		D3D12_TEXTURE_ADDRESS_MODE_WRAP,  // addressU
		D3D12_TEXTURE_ADDRESS_MODE_WRAP,  // addressV
		D3D12_TEXTURE_ADDRESS_MODE_WRAP // addressW
#elif defined(Ex1_Aniso)
		0, // shaderRegister
		D3D12_FILTER_ANISOTROPIC, // filter
		D3D12_TEXTURE_ADDRESS_MODE_WRAP,  // addressU
		D3D12_TEXTURE_ADDRESS_MODE_WRAP,  // addressV
		D3D12_TEXTURE_ADDRESS_MODE_WRAP // addressW
#elif defined(Ex1_Border)
		0, // shaderRegister
		D3D12_FILTER_MIN_MAG_MIP_POINT, // filter
		D3D12_TEXTURE_ADDRESS_MODE_BORDER,  // addressU
		D3D12_TEXTURE_ADDRESS_MODE_BORDER,  // addressV
		D3D12_TEXTURE_ADDRESS_MODE_BORDER // addressW
#elif defined(Ex1_Clamp) || defined(Ex4_Trans) || defined(Ex4_Shader)
		0, // shaderRegister
		D3D12_FILTER_MIN_MAG_MIP_POINT, // filter
		D3D12_TEXTURE_ADDRESS_MODE_CLAMP,  // addressU
		D3D12_TEXTURE_ADDRESS_MODE_CLAMP,  // addressV
		D3D12_TEXTURE_ADDRESS_MODE_CLAMP // addressW
#elif defined(Ex1_Mirror)
		0, // shaderRegister
		D3D12_FILTER_MIN_MAG_MIP_POINT, // filter
