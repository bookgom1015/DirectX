#include "NormalMap/NormalMapApp.h"
...
#if defined(Ex5)
	mCommandList->SetPipelineState(mPSOs["bricks"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Bricks]);
...
void NormalMapApp::AnimateMaterials(const GameTimer& gt) {
#if defined(Ex3)
	Material* mat = mMaterials["tile0"].get();
	
	XMStoreFloat4x4(&mat->MatTransform, XMMatrixRotationZ(gt.TotalTime() * 0.1f));
	
	mat->NumFramesDirty = gNumFrameResources;
#elif defined(Ex5)
	float& textu = mWavesRitem->TexTransform(3, 0);
	float& textv = mWavesRitem->TexTransform(3, 1);

	textu -= 0.05f * gt.DeltaTime();
	textv += 0.1f * gt.DeltaTime();
	...
	mWavesRitem->TexTransform(3, 0) = textu;
	mWavesRitem->TexTransform(3, 1) = textv;

	mWavesRitem->NumFramesDirty = gNumFrameResources;

	Material* wavesMat = mMaterials["tile0"].get();
	
	float& mattu = wavesMat->MatTransform(3, 0);
	float& mattv = wavesMat->MatTransform(3, 1);
	
	mattu += 0.15f * gt.DeltaTime();
	mattv -= 0.02f * gt.DeltaTime();
	...
	wavesMat->MatTransform(3, 0) = mattu;
	wavesMat->MatTransform(3, 1) = mattv;
	
	wavesMat->NumFramesDirty = gNumFrameResources;
#endif
}
...
void NormalMapApp::UpdateMaterialBuffer(const GameTimer& gt) {
	...
	for (auto& e : mMaterials) {
		...
		if (mat->NumFramesDirty > 0) {
			...
			matData.DispMapIndex = mat->DispSrvHeapIndex;
...
void NormalMapApp::UpdateCubeMapFacePassCBs() {
	for (int i = 0; i < 6; ++i) {
...
void NormalMapApp::BuildDescriptorHeaps() {
	...
	std::vector<ComPtr<ID3D12Resource>> tex2DList = {
		...
		mTextures["bricksDispMap"]->Resource,
		mTextures["tileDiffuseMap"]->Resource,
		mTextures["tileNormalMap"]->Resource,
		mTextures["tileDispMap"]->Resource,
		mTextures["defaultDiffuseMap"]->Resource,
		mTextures["defaultNormalMap"]->Resource
#elif defined(Ex5)
		mTextures["bricksDiffuseMap"]->Resource,
		mTextures["bricksNormalMap"]->Resource,
		mTextures["waves01NormalMap"]->Resource,
		mTextures["waves02NormalMap"]->Resource,
		mTextures["waves01DispMap"]->Resource,
		mTextures["waves02DispMap"]->Resource,
		mTextures["defaultDiffuseMap"]->Resource,
		mTextures["defaultNormalMap"]->Resource,
		mTextures["bricksDispMap"]->Resource
#else
...
void NormalMapApp::BuildShadersAndInputLayout() {
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\Default.hlsl", defines, "VS", "vs_5_1");
	...
	mShaders["bricksVS"] = d3dUtil::CompileShader(L"Shaders\\Bricks.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["bricksHS"] = d3dUtil::CompileShader(L"Shaders\\Bricks.hlsl", nullptr, "HS", "hs_5_1");
	mShaders["bricksDS"] = d3dUtil::CompileShader(L"Shaders\\Bricks.hlsl", nullptr, "DS", "ds_5_1");
	mShaders["bricksPS"] = d3dUtil::CompileShader(L"Shaders\\Bricks.hlsl", nullptr, "PS", "ps_5_1");
#endif
...
void NormalMapApp::BuildPSOs() {
	...
	//wavesPsoDesc.GS = {
	//	reinterpret_cast<BYTE*>(mShaders["wavesGS"]->GetBufferPointer()),
	//	mShaders["wavesGS"]->GetBufferSize()
	//};
	wavesPsoDesc.PS = {
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC bricksPsoDesc = opaquePsoDesc;
	bricksPsoDesc.VS = {
		reinterpret_cast<BYTE*>(mShaders["bricksVS"]->GetBufferPointer()),
		mShaders["bricksVS"]->GetBufferSize()
	};
	bricksPsoDesc.HS = {
		reinterpret_cast<BYTE*>(mShaders["bricksHS"]->GetBufferPointer()),
		mShaders["bricksHS"]->GetBufferSize()
	};
	bricksPsoDesc.DS = {
		reinterpret_cast<BYTE*>(mShaders["bricksDS"]->GetBufferPointer()),
		mShaders["bricksDS"]->GetBufferSize()
	};
	bricksPsoDesc.PS = {
		reinterpret_cast<BYTE*>(mShaders["bricksPS"]->GetBufferPointer()),
		mShaders["bricksPS"]->GetBufferSize()
	};
	bricksPsoDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_PATCH;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&bricksPsoDesc, IID_PPV_ARGS(&mPSOs["bricks"])));
#endif
}
...
#elif defined(Ex5)
	XMStoreFloat4x4(&tile0->MatTransform, XMMatrixScaling(2.0f, 2.0f, 1.0f));
	tile0->DiffuseSrvHeapIndex = 6;
	tile0->NormalSrvHeapIndex = 2;
	tile0->DispSrvHeapIndex = 4;
#else
	tile0->DiffuseSrvHeapIndex = 2;
	tile0->NormalSrvHeapIndex = 3;
#endif
#if defined(Ex5)
	tile0->DiffuseAlbedo = XMFLOAT4(0.005f, 0.6f, 0.65f, 1.0f);
	tile0->FresnelR0 = XMFLOAT3(0.8f, 0.8f, 0.8f);
...
#if defined(Ex2) || defined(Ex5)
	mirror0->DiffuseSrvHeapIndex = 6;
	mirror0->NormalSrvHeapIndex = 7;
#else
...
#if defined(Ex5)
	mWavesRitem = gridRitem.get();

	mRitemLayer[(int)RenderLayer::Waves].push_back(gridRitem.get());
#else
...
#if defined(Ex5)
		leftCylRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
#else
...
#if defined(Ex5)
		rightCylRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
#else
...
#if defined(Ex5)
		mRitemLayer[(int)RenderLayer::Bricks].push_back(leftCylRitem.get());
		mRitemLayer[(int)RenderLayer::Bricks].push_back(rightCylRitem.get());
#else
...
void NormalMapApp::DrawSceneToCubeMap() {
...
void NormalMapApp::BuildCubeFaceCamera(float x, float y, float z) {
