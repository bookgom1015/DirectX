#include "DrawShapes/ShapesApp.h"
...
void ShapesApp::OnResize() {
	...
	// The window resized, so update the aspect ration and recompute the projection matrix.
...
void ShapesApp::Draw(const GameTimer& gt) {
	...
	// Reusing the command list resues memory.
	if (mIsWireframe) {
	...
	// Clear the back buffer we are going th render to.
	...
	// set until the GPU finished processing all the commands prior to this Signal().
...
void ShapesApp::UpdateObjectCBs(const GameTimer& gt) {
	...
	for (auto& e : mAllRitems) {
		...
		// This needs to be trakced per frame resource.
...
void ShapesApp::BuildDescriptorHeaps() {
	...
	// Save an offset to thte start of the pass CBVs. These are the last 3 descriptors.
...
void ShapesApp::BuildConstantBufferViews() {
	...
	for (int frameIndex = 0; frameIndex < gNumFrameResources; frameIndex++) {
		...
		for (UINT i = 0; i < objCount; i++) {
			...
			// Offset to the ith objet constatnt buffer in the buffer.
...
void ShapesApp::BuildShadersAndInputLayout() {
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\const_color_vs.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders\\const_color_ps.hlsl", nullptr, "PS", "ps_5_1");
...
void ShapesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems) {
	...
	for (size_t i = 0; i < ritems.size(); i++) {
		...
		UINT cbIndex = 0;
		cbIndex += i * 16;
		cmdList->SetGraphicsRoot32BitConstants(0, 16, &ri->World, cbIndex);
#else
...
void ShapesApp::BuildSkullGeometry() {
	...
	for (UINT i = 0; i < vcount; i++) {
		...
		file >> vertices[i].Color.x >> vertices[i].Color.y >> vertices[i].Color.z;
