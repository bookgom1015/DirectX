...
void Ssao::BuildDescriptors(
	...
	mhAmbientMap1CpuSrv = hCpuSrv.Offset(1, inCbvSrvUavDescriptorSize);
	mhNormalMapCpuSrv = hCpuSrv.Offset(1, inCbvSrvUavDescriptorSize);
	mhDepthMapCpuSrv = hCpuSrv.Offset(1, inCbvSrvUavDescriptorSize);
	mhRandomVectorMapCpuSrv = hCpuSrv.Offset(1, inCbvSrvUavDescriptorSize);

	mhAmbientMap0GpuSrv = hGpuSrv;
	mhAmbientMap1GpuSrv = hGpuSrv.Offset(1, inCbvSrvUavDescriptorSize);
	mhNormalMapGpuSrv = hGpuSrv.Offset(1, inCbvSrvUavDescriptorSize);
	mhDepthMapGpuSrv = hGpuSrv.Offset(1, inCbvSrvUavDescriptorSize);
	mhRandomVectorMapGpuSrv = hGpuSrv.Offset(1, inCbvSrvUavDescriptorSize);

	mhNormalMapCpuRtv = hCpuRtv;
	mhAmbientMap0CpuRtv = hCpuRtv.Offset(1, inRtvDescriptorSize);
	mhAmbientMap1CpuRtv = hCpuRtv.Offset(1, inRtvDescriptorSize);

	//  Create the descriptors
	RebuildDescriptors(inDepthStencilBuffer);
}

void Ssao::RebuildDescriptors(ID3D12Resource* inDepthStencilBuffer) {
...
void Ssao::BlurAmbientMap(ID3D12GraphicsCommandList* inCmdList, bool inHorzBlur) {
	...
	inCmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(output,
		D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_RENDER_TARGET));
