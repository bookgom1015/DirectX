...
#if defined(Ex1)
	carRitem->Sphere = carRitem->Geo->DrawArgs["car"].Sphere;
#else
	carRitem->AABB = carRitem->Geo->DrawArgs["car"].AABB;
#endif
...
void PickingApp::Pick(int sx, int sy) {
	...
	for (auto ri : mRitemLayer[(int)RenderLayer::Opaque]) {
		...
		float tmin = 0.0f;
#if defined(Ex1)
		if (ri->Sphere.Intersects(rayOrigin, rayDir, tmin)) {
#else
		if (ri->AABB.Intersects(rayOrigin, rayDir, tmin)) {
#endif
