#include "StencilDemo/StencilApp.h"
...
void StencilApp::Draw(const GameTimer& gt) {
...
#if defined(Ex3)
	mCommandList->ClearDepthStencilView(DepthStencilView(), D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 1, 0, nullptr);
#else
	...
	DrawRenderItems(mCommandList.Get(), mRitemLayer[static_cast<int>(RenderLayer::ReflectedShadow)]);
#endif
...
#if defined(Ex11)
	XMStoreFloat4x4(&mReflectedShadowRitem->World, skullWorld * S * R * shadowOffsetY);
#endif
#endif
...
#if defined(Ex11)
	mReflectedShadowRitem->NumFramesDirty = gNumFrameResources;
#endif
}
...
void StencilApp::LoadTextures() {
	...
	bricksTex->Filename = L"./../../../../Assets/Textures/bricks3.dds";
...
#if defined(Ex11)
	D3D12_DEPTH_STENCIL_DESC reflectedShadowDDS;
	reflectedShadowDDS.DepthEnable = true;
	reflectedShadowDDS.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
	reflectedShadowDDS.DepthFunc = D3D12_COMPARISON_FUNC_LESS;
	reflectedShadowDDS.StencilEnable = true;
	reflectedShadowDDS.StencilReadMask = 0xff;
	reflectedShadowDDS.StencilWriteMask = 0xff;

	reflectedShadowDDS.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	reflectedShadowDDS.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	reflectedShadowDDS.FrontFace.StencilPassOp = D3D12_STENCIL_OP_INCR;
	reflectedShadowDDS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;

	reflectedShadowDDS.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	reflectedShadowDDS.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	reflectedShadowDDS.BackFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
	reflectedShadowDDS.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;

	D3D12_GRAPHICS_PIPELINE_STATE_DESC reflectedShadowPsoDesc = transparentPsoDesc;
	reflectedShadowPsoDesc.DepthStencilState = reflectedShadowDDS;
...
void StencilApp::BuildRenderItems() {
	...
	mRitemLayer[static_cast<int>(RenderLayer::Opaque)].push_back(floorRitem.get());
	...
	mRitemLayer[static_cast<int>(RenderLayer::Opaque)].push_back(wallsRitem.get());
	...
	*reflectedSkullRitem = *skullRitem;
	...
	mRitemLayer[static_cast<int>(RenderLayer::Reflected)].push_back(reflectedSkullRitem.get());
	...
	*shadowedSkullRitem = *skullRitem;
	...
	mRitemLayer[static_cast<int>(RenderLayer::Shadow)].push_back(shadowedSkullRitem.get());
	...
	mRitemLayer[static_cast<int>(RenderLayer::Mirrors)].push_back(mirrorRitem.get());
	mRitemLayer[static_cast<int>(RenderLayer::Transparent)].push_back(mirrorRitem.get());
	...
	*reflectedFloorRitem = *floorRitem;
	...
	mRitemLayer[static_cast<int>(RenderLayer::Reflected)].push_back(reflectedFloorRitem.get());
	...
	*reflectedShadowRitem = *shadowedSkullRitem;
	...
	reflectedShadowRitem->Mat = mMaterials["shadowMat"].get();
	mReflectedShadowRitem = reflectedShadowRitem.get();
	mRitemLayer[static_cast<int>(RenderLayer::ReflectedShadow)].push_back(reflectedShadowRitem.get());
#endif
