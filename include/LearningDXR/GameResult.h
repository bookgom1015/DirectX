...
static GameResult __STATIC_GAMERESULT_FAIL(E_FAIL);
	...
	{															\
		...
		if (FAILED(__return_hr)) {								\
			...
			__wsstream_RIF <<  L#__statement << L" failed;";	\
	...
	{																							\
		...
		if (FAILED(__return_hr)) {																\
			...
			__wsstream_RIFDXDBG <<  L#__statement << L" failed; " << __message->pDescription;	\
	...
	#define ReturnIfFalse(__statement)								\
	{																\
		bool __result = (__statement);								\
		if (!__result) {											\
			std::wstringstream __wsstream_RIFM;						\
			__wsstream_RIFM << L#__statement << L" is invalid; ";	\
