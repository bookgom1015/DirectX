...
static GameResult __STATIC_GAMERESULT_FAIL(E_FAIL);
	...
	{																					\
		...
		__wsstream_RGR << __FILE__ << L"; line: " << __LINE__ << L"; " << __message;	\
		WErrln(__wsstream_RGR.str());													\
	...
	{																											\
		...
		if (FAILED(__return_hr)) {																				\
			...
			__wsstream_RIF <<  __FILE__ << L"; line: " << __LINE__ << L"; " << L#__statement << L" failed;";	\
			WErrln(__wsstream_RIF.str());																		\
	...
	{																																			\
		...
		if (FAILED(__return_hr)) {																												\
			...
			__wsstream_RIFDXDBG <<  __FILE__ << L"; line: " << __LINE__ << L"; " <<	L#__statement << L" failed; " << __message->pDescription;	\
			WErrln(__wsstream_RIFDXDBG.str());																									\
			return GameResult(__return_hr);																										\
		}																																		\
	}
#endif
	...
	{																														\
		...
		if (!__result) {																									\
			...
			__wsstream_RIFM <<  __FILE__ << L"; line: " << __LINE__ << L"; " <<	L#__statement << L" is invalid; " << __msg;	\
			WErrln(__wsstream_RIFM.str());																					\
...
#ifndef ShowRemovedReason
	#define ShowRemovedReason(__renderer)											\
	{																				\
		HRESULT reason = __renderer->GetDeviceRemovedReason();						\
		if (FAILED(reason)) {														\
			std::wstringstream wsstream;											\
			wsstream << L"Device removed reason code: 0x" << std::hex << reason;	\
			WLogln(wsstream.str());													\
		}																			\
	}
#endif