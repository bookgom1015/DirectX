...
VertexOut VS(VertexIn vin, uint instanceID : SV_InstanceID) {
	...
	vout.TexTrans	= instData.TexTransform;
...
HullOut HS(InputPatch<VertexOut, 3> p,
	...
	hout.World		= p[i].World;
	hout.TexTrans	= p[i].TexTrans;
...
DomainOut DS(PatchTess patchTess,
	...
	float4x4 world = tri[0].World;
	float4x4 texTrans = tri[0].TexTrans;

	dout.MatIndex = tri[0].MatIndex;

	// Fetch the material data.
	MaterialData matData = gMaterialData[tri[0].MatIndex];
	...
	dout.TexC1 = mul(texC, world).xy;
	dout.TexC2 = mul(texC, texTrans).xy;
	...
	int dispMapIndex1 = matData.DispMapIndex;
	int dispMapIndex2 = matData.DispMapIndex + 1;
