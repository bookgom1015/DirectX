...
float3 CubicBezierSum(const OutputPatch<HullOut, 10> bezpatch, float u, float v, float w)
{
    float3 sum = float3(0.0f, 0.0f, 0.0f);

	sum += u * u * u * bezpatch[0].PosL;
	sum += v * v * v * bezpatch[1].PosL;
	sum += w * w * w * bezpatch[2].PosL;
	sum += 3 * u * u * v * bezpatch[3].PosL;
	sum += 3 * u * u * w * bezpatch[4].PosL;
	sum += 3 * u * v * v * bezpatch[5].PosL;
	sum += 3 * v * v * w * bezpatch[6].PosL;
	sum += 3 * v * w * w * bezpatch[7].PosL;
	sum += 3 * u * w * w * bezpatch[8].PosL;
	sum += 6 * u * v * w * bezpatch[9].PosL;
